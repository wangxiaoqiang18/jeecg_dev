...
<head>
	<title>图表配置</title>
	<script type="text/javascript" src="${basepath}plug-in/mutiLang/zh-cn.js"></script><script type="text/javascript" src="${basepath}plug-in/jquery/jquery-1.8.3.js"></script><script type="text/javascript" src="${basepath}plug-in/jquery/jquery.cookie.js" ></script><script type="text/javascript" src="${basepath}plug-in/jquery-plugs/storage/jquery.storageapi.min.js" ></script><script type="text/javascript" src="${basepath}plug-in/jquery-plugs/i18n/jquery.i18n.properties.js"></script><script type="text/javascript" src="${basepath}plug-in/tools/dataformat.js"></script><link id="easyuiTheme" rel="stylesheet" href="${basepath}plug-in/easyui/themes/metrole/easyui.css" type="text/css"></link><link id="easyuiTheme" rel="stylesheet" href="${basepath}plug-in/easyui/themes/metrole/main.css" type="text/css"></link><link id="easyuiTheme" rel="stylesheet" href="${basepath}plug-in/easyui/themes/metrole/icon.css" type="text/css"></link><link rel="stylesheet" type="text/css" href="${basepath}plug-in/accordion/css/accordion.css"><link rel="stylesheet" type="text/css" href="${basepath}plug-in/accordion/css/icons.css"><script type="text/javascript" src="${basepath}plug-in/easyui/jquery.easyui.min.1.3.2.js"></script><script type="text/javascript" src="${basepath}plug-in/easyui/locale/zh-cn.js"></script><script type="text/javascript" src="${basepath}plug-in/tools/syUtil.js"></script><script type="text/javascript" src="${basepath}plug-in/easyui/extends/datagrid-scrollview.js"></script><script type="text/javascript" src="${basepath}plug-in/mutitables/datagrid-filter.js"></script><script type="text/javascript" src="${basepath}plug-in/My97DatePicker/WdatePicker.js"></script><link rel="stylesheet" href="${basepath}plug-in/tools/css/metrole/common.css" type="text/css"></link><link rel="stylesheet" href="${basepath}plug-in/ace/css/font-awesome.css" type="text/css"></link><script type="text/javascript" src="${basepath}plug-in/lhgDialog/lhgdialog.min.js?skin=metrole"></script><script type="text/javascript" src="${basepath}plug-in/ace/js/bootstrap-tab.js"></script><script type="text/javascript" src="${basepath}plug-in/layer/layer.js"></script><script type="text/javascript" src="${basepath}plug-in/tools/curdtools.js"></script><script type="text/javascript" src="${basepath}plug-in/tools/easyuiextend.js"></script><script type="text/javascript" src="${basepath}plug-in/jquery-plugs/hftable/jquery-hftable.js"></script><script type="text/javascript" src="${basepath}plug-in/tools/json2.js" ></script>
	<script type="text/javascript">
        $(document).ready(function(){
            $('#tt').tabs({
                onSelect:function(title){
...
<body style="overflow-x: hidden;">
<form id="formobj"  action="jformGraphreportHeadController.do?doAdd" name="formobj" method="post"><input type="hidden" id="btn_sub" class="btn_sub"/>
	...
	<table cellpadding="0" cellspacing="1" class="formtable">
		...
		<tr>
			...
			<td class="value">
				<select name="isShowList"> <option  value="">   </option> <option  value="Y">是 </option> <option  value="N">否 </option></select>
				<span class="Validform_checktip"></span>
				<label class="Validform_label" style="display: none;">是否显示明细</label>
			</td>
		</tr>
	...
	<div style="width: auto;">
		...
		<div id="tt" tabPosition="top" border=flase style="margin:0px;padding:0px;overflow-x:hidden;width:auto;" class="easyui-tabs" fit="false"><div title="图表配置" iconCls="icon-search"  href="jformGraphreportHeadController.do?jformGraphreportItemList&id=" style="margin:0px;padding:0px;overflow-x:hidden;overflow-y:auto;width=auto;"></div></div>
	</div>
	<link rel="stylesheet" href="${basepath}plug-in/Validform/css/metrole/style.css" type="text/css"/><link rel="stylesheet" href="${basepath}plug-in/Validform/css/metrole/tablefrom.css" type="text/css"/><script type="text/javascript" src="plug-in/Validform/js/Validform_v5.3.1_min_zh-cn.js"></script><script type="text/javascript" src="plug-in/Validform/js/Validform_Datatype_zh-cn.js"></script><script type="text/javascript" src="plug-in/Validform/js/datatype_zh-cn.js"></script><SCRIPT type="text/javascript" src="plug-in/Validform/plugin/passwordStrength/passwordStrength-min.js"></SCRIPT><script src="plug-in/layer/layer.js"></script><script type="text/javascript">var subDlgIndex = null;$(function(){$("#formobj").Validform({tiptype:function(msg,o,cssctl){if(o.type == 3){layer.open({title:'提示信息',content:msg,icon:5,shift:6,btn:false,shade:false,time:5000,cancel:function(index){o.obj.focus();layer.close(index);},yes:function(index){o.obj.focus();layer.close(index);}})}},btnSubmit:"#btn_sub",btnReset:"#btn_reset",ajaxPost:true,beforeSubmit:function(curform){var tag=false;subDlgIndex = $.dialog({content: '正在加载中',zIndex:19910320,lock:true,width:100,height:50,opacity:0.3,title:'提示',cache:false});var infoTable = subDlgIndex.DOM.t.parent().parent().parent();infoTable.parent().append('<div id="infoTable-loading" style="text-align:center;"><img src="plug-in/layer/skin/default/loading-0.gif"/></div>');infoTable.css('display','none');},usePlugin:{passwordstrength:{minLen:6,maxLen:18,trigger:function(obj,error){if(error){obj.parent().next().find(".Validform_checktip").show();obj.find(".passwordStrength").hide();}else{$(".passwordStrength").show();obj.parent().next().find(".Validform_checktip").hide();}}}},callback:function(data){if(subDlgIndex && subDlgIndex != null){$('#infoTable-loading').hide();subDlgIndex.close();}var win = frameElement.api.opener;if(data.success==true){frameElement.api.close();win.tip(data.msg);}else{if(data.responseText==''||data.responseText==undefined){$.messager.alert('错误', data.msg);$.Hidemsg();}else{try{var emsg = data.responseText.substring(data.responseText.indexOf('错误描述'),data.responseText.indexOf('错误信息')); $.messager.alert('错误',emsg);$.Hidemsg();}catch(ex){$.messager.alert('错误',data.responseText+"");$.Hidemsg();}} return false;}win.reloadTable();}});});</script></form>
<!-- 添加 附表明细 模版 -->
<table style="display:none">
	<tbody id="add_jformGraphreportItem_table_template">
	<tr>
		...
		<td align="left">
			<select name="jformGraphreportItemList[#index#].fieldType" style="width:100px"> <option  value="" selected="selected">   </option> <option  value="Integer">数值类型 </option> <option  value="Date">日期类型 </option> <option  value="String">字符类型 </option> <option  value="Long">长整型 </option></select>
			<label class="Validform_label" style="display: none;">字段类型</label>
		</td>
		<td align="left">
			<select name="jformGraphreportItemList[#index#].isShow" style="width:100px"> <option  value="" selected="selected">   </option> <option  value="Y">是 </option> <option  value="N">否 </option></select>
			<label class="Validform_label" style="display: none;">是否显示</label>
		</td>
		<td align="left">
			<select name="jformGraphreportItemList[#index#].searchFlag" style="width:100px"> <option  value="" selected="selected">   </option> <option  value="Y">是 </option> <option  value="N">否 </option></select>
			<label class="Validform_label" style="display: none;">是否查询</label>
		</td>
		<td align="left">
			<select name="jformGraphreportItemList[#index#].searchMode" style="width:100px"> <option  value="" selected="selected">   </option> <option  value="single">单条件查询 </option> <option  value="group">范围查询 </option></select>
			<label class="Validform_label" style="display: none;">查询模式</label>
		</td>
		<td align="left">
			<input name="jformGraphreportItemList[#index#].dictCode" maxlength="500"
		...
		<td align="left">
			<select name="jformGraphreportItemList[#index#].isGraph" style="width:100px"> <option  value="" selected="selected">   </option> <option  value="Y">是 </option> <option  value="N">否 </option></select>
			<label class="Validform_label" style="display: none;">显示图表</label>
		</td>
		<td align="left">
			<select name="jformGraphreportItemList[#index#].graphType" style="width:100px"> <option  value="" selected="selected">   </option> <option  value="spline">曲线图 </option> <option  value="column">柱状图 </option> <option  value="pie">饼图 </option></select>
			<label class="Validform_label" style="display: none;">图表类型</label>
		</td>
		<td align="left">
			<input name="jformGraphreportItemList[#index#].graphName" maxlength="100"
